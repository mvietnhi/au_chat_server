...
export default () => {
  ...
  api.get("/profile", async (req: any, res, next) => {
    try {
      const { id } = req.user;

      const profile = await userService.getSingleUser(id);

      returnSuccess(200, res, "Get profile", profile);
    } catch (error) {
      next(error);
    }
  });

  api.put("/profile", async (req: any, res, next) => {
    try {
      const { id } = req.user;
      const updateProfile: UpdateProfile = req.body;

      const profile = await userService.updateProfile(id, updateProfile);
